...
public abstract class TitleImageDrawFormat<T> extends ImageResTitleDrawFormat<T> {
   ...
   private Rect rect;
    ...
    public int measureHeight(TableData<T> tableData, TableConfig config) {
        int imgHeight = super.measureHeight(tableData,config);
        int textHeight = textDrawFormat.measureHeight(tableData,config);
    ...
    public void draw(Canvas c, TableData<T> tableData, Column column, Rect rect, TableConfig config) {
        ...
        if(getBitmap(column) == null){
            textDrawFormat.draw(c,tableData,column,rect,config);
            return;
        }
        ...
        switch (direction){
            case LEFT:
                    ...
                    super.draw(c,tableData,column,this.rect,config);
                     ...
                     textDrawFormat.draw(c,tableData,column,this.rect,config);
            ...
            case RIGHT:
                ...
                break;
            case TOP:
                height = (int) (measureHeight(tableData,config)*config.getZoom());
                ...
                textDrawFormat.draw(c,tableData,column, this.rect,config);
                textHeight = (int) (textDrawFormat.measureHeight(tableData,config)*config.getZoom());
